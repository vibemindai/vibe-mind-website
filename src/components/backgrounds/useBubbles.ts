...
export function useBubbles(
  ...
  const createBubble = useCallback((width: number, height: number, startAtBottom: boolean = false): Bubble => {
  ...
  const updateBubbles = useCallback((width: number, height: number, time: number) => {
    ...
    for (let i = 0; i < bubbles.length; i++) {
      ...
      const swayX = Math.sin(time * bubble.swayFrequency + bubble.swayOffset) * bubble.swayAmplitude;
      bubble.x += swayX * 0.01;

      // Breathing/pulse effect (±5% size)
      bubble.radius = bubble.baseRadius * (1 + Math.sin(time * 0.001 + bubble.pulseOffset) * 0.05);
